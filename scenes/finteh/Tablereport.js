...
export default function Tablereport({data,totalSum}) {
  ...
  return (
    <div className='text-center' id='top'>
        ...
        <table className='border-2 border-black w-full m-4'>
            ...
            <tbody className='border-2 border-black w-full '>
                {data ? data.map(item =>
                           ...
                           <td className='border-2 border-black'>{item.cart.map(cartItem =>
                            <div key={Math.random()} className='flex border-2 w-full flex-wrap'>
        ...
        <div id="down" className='text-right m-4 text-3xl'>Total sum per: {totalSum}</div>
